  ...
  return (
    ...
    {/* INPUT DE BUSQUEDA */}
      <header className='w-full h-96 items-center justify-center object-cover' style={{ backgroundImage: "url('/image/fondomangas.png')", objectFit: "contain" }}>
        <h2 className='p-5   text-center text-white text-4xl font-semibold md:font-bold md:text-6xl '>MANGAS</h2>
      ...
      <main className='w-full min-h-screen flex flex-col justify-around bg-slate-100  rounded-3xl' >
        {/* RENDERIZACION DE CATEGORIAS */}
        <div className='flex items-center justify-center'>
          {categories.map((category) => (
            <button
              onClick={() => checkeados(category._id)}
              key={category._id}
              className='rounded-full w-24 p-3 m-2 mb-2 transition-opacity hover:opacity-70 focus:outline-none active:bg-opacity-70'
